...
template < int SIZE > class Unweighted_Gradient_SCALAR_None
{
private:
...
public:
    explicit Unweighted_Gradient_SCALAR_None (float *u_in, float *F_in,
                                              float
                                              *one_over_h_in):_local_u (u_in),
    _local_F (F_in), _local_one_over_h (one_over_h_in)
  {
  }
...
template < int SIZE > class Unweighted_Gradient_SSE_None
{
private:
  // Generate Variables Here
  float *_local_u;
...
public:
    explicit Unweighted_Gradient_SSE_None (float *u_in, float *F_in,
                                           float
                                           *one_over_h_in):_local_u (u_in),
    _local_F (F_in), _local_one_over_h (one_over_h_in)
  {
...
template < int SIZE > class Unweighted_Gradient_AVX_None
{
private:
  // Generate Variables Here
  float *_local_u;
...
public:
    explicit Unweighted_Gradient_AVX_None (float *u_in, float *F_in,
                                           float
                                           *one_over_h_in):_local_u (u_in),
    _local_F (F_in), _local_one_over_h (one_over_h_in)
  {
...
template < int SIZE > class Unweighted_Gradient_NEON_None
{
private:
  // Generate Variables Here
  float *_local_u;
...
public:
    explicit Unweighted_Gradient_NEON_None (float *u_in, float *F_in,
                                            float
                                            *one_over_h_in):_local_u (u_in),
    _local_F (F_in), _local_one_over_h (one_over_h_in)
  {
...
template < int SIZE > class Unweighted_Gradient_MIC_None
{
private:
  // Generate Variables Here
  float *_local_u;
...
public:
    explicit Unweighted_Gradient_MIC_None (float *u_in, float *F_in,
                                           float
                                           *one_over_h_in):_local_u (u_in),
    _local_F (F_in), _local_one_over_h (one_over_h_in)
  {
...
main (int argc, char *argv[])
{
  ...
  {
    std::cout << "Running Thread Test for Unweighted_Gradient " << std::endl;
...
//=======================================================

    {
      ...
      Unweighted_Gradient_SCALAR_None < data_size > op ((float *) &u,
                                                        (float *) &F,
                                                        (float *) &one_over_h);
      ...
      for (int t = threads; t <= threads_max;
           t += std::max < int >(((threads_max - threads) / 30), 1))
        {
          ...
          MT_Streaming_Kernels::Kernel_Serial_Base_Helper <
            Unweighted_Gradient_SCALAR_None < data_size > >helper (op,
                                                                   data_size,
                                                                   t);
...
//=======================================================
#ifdef ENABLE_SSE_INSTRUCTION_SET
    {
      ...
      Unweighted_Gradient_SSE_None < data_size > op ((float *) &u, (float *) &F,
                                                     (float *) &one_over_h);
      ...
      for (int t = threads; t <= threads_max;
           t += std::max < int >(((threads_max - threads) / 30), 1))
        {
          std::cout << "Running Test with " << t << " threads." << std::endl;
          MT_Streaming_Kernels::Kernel_Serial_Base_Helper <
            Unweighted_Gradient_SSE_None < data_size > >helper (op, data_size,
                                                                t);
...
//=======================================================
#ifdef ENABLE_AVX_INSTRUCTION_SET
    {
      ...
      Unweighted_Gradient_AVX_None < data_size > op ((float *) &u, (float *) &F,
                                                     (float *) &one_over_h);
      ...
      for (int t = threads; t <= threads_max;
           t += std::max < int >(((threads_max - threads) / 30), 1))
        {
          std::cout << "Running Test with " << t << " threads." << std::endl;
          MT_Streaming_Kernels::Kernel_Serial_Base_Helper <
            Unweighted_Gradient_AVX_None < data_size > >helper (op, data_size,
                                                                t);
...
//=======================================================
#ifdef ENABLE_NEON_INSTRUCTION_SET
    {
      ...
      Unweighted_Gradient_NEON_None < data_size > op ((float *) &u,
                                                      (float *) &F,
                                                      (float *) &one_over_h);
      ...
      for (int t = threads; t <= threads_max;
           t += std::max < int >(((threads_max - threads) / 30), 1))
        {
          std::cout << "Running Test with " << t << " threads." << std::endl;
          MT_Streaming_Kernels::Kernel_Serial_Base_Helper <
            Unweighted_Gradient_NEON_None < data_size > >helper (op, data_size,
                                                                 t);
...
//=======================================================
#ifdef ENABLE_MIC_INSTRUCTION_SET
    {
      ...
      Unweighted_Gradient_MIC_None < data_size > op ((float *) &u, (float *) &F,
                                                     (float *) &one_over_h);
      ...
      for (int t = threads; t <= threads_max;
           t += std::max < int >(((threads_max - threads) / 30), 1))
        {
          std::cout << "Running Test with " << t << " threads." << std::endl;
          MT_Streaming_Kernels::Kernel_Serial_Base_Helper <
            Unweighted_Gradient_MIC_None < data_size > >helper (op, data_size,
                                                                t);
