...
template < int SIZE > class Symmetric_Definite_Projection_SCALAR_None
{
private:
...
public:
    explicit Symmetric_Definite_Projection_SCALAR_None (float *A_in,
                                                        float
                                                        *Apd_in):_local_A
    (A_in), _local_Apd (Apd_in)
  {
  }
  void Execute (int index)
  {
    ...
    typedef float (&fullArray1)[SIZE][6][16];
    typedef float (&fullArray2)[SIZE][6][16];

    // full array extractions
    //refArray _rA = reinterpret_cast < refArray >(*_A);
    fullArray1 _rA = reinterpret_cast < fullArray1 > (*_local_A);
    fullArray2 _rApd = reinterpret_cast < fullArray2 > (*_local_Apd);

    const int ChunkSize = 1;
    // chunk typedef
    //typedef int (&refArray1)[3][8];
    typedef float (&refArray1)[6][16];
    typedef float (&refArray2)[6][16];
    ...
    for (int chunk_offset = 0; chunk_offset < 16; chunk_offset += ChunkSize)
      {
        ...
        refArray2 Apdk =
          reinterpret_cast < refArray2 > (_rApd[index][0][chunk_offset]);
...
template < int SIZE > class Symmetric_Definite_Projection_SSE_None
{
private:
  // Generate Variables Here
  float *_local_A;
...
public:
    explicit Symmetric_Definite_Projection_SSE_None (float *A_in,
                                                     float
                                                     *Apd_in):_local_A (A_in),
    _local_Apd (Apd_in)
  {
  }
  void Execute (int index)
...
template < int SIZE > class Symmetric_Definite_Projection_AVX_None
{
private:
  // Generate Variables Here
  float *_local_A;
...
public:
    explicit Symmetric_Definite_Projection_AVX_None (float *A_in,
                                                     float
                                                     *Apd_in):_local_A (A_in),
    _local_Apd (Apd_in)
  {
...
template < int SIZE > class Symmetric_Definite_Projection_NEON_None
{
private:
  // Generate Variables Here
  float *_local_A;
...
public:
    explicit Symmetric_Definite_Projection_NEON_None (float *A_in,
                                                      float
                                                      *Apd_in):_local_A (A_in),
    _local_Apd (Apd_in)
  {
...
template < int SIZE > class Symmetric_Definite_Projection_MIC_None
{
private:
  // Generate Variables Here
  float *_local_A;
...
public:
    explicit Symmetric_Definite_Projection_MIC_None (float *A_in,
                                                     float
                                                     *Apd_in):_local_A (A_in),
    _local_Apd (Apd_in)
  {
...
main (int argc, char *argv[])
{
  ...
  {
    std::
      cout << "Running Thread Test for Symmetric_Definite_Projection " << std::
      endl;
...
//=======================================================
    ...
    start_timer ();
    typedef T (&A_type)[data_size][6][16];
    A_type A =
      reinterpret_cast < A_type >
      (*((T *) (_mm_malloc (data_size * 6 * 16 * sizeof (T), 64))));
    typedef T (&Apd_type)[data_size][6][16];
    Apd_type Apd =
      reinterpret_cast < Apd_type >
      (*((T *) (_mm_malloc (data_size * 6 * 16 * sizeof (T), 64))));


    for (int __a = 0; __a < data_size; __a++)
      for (int __b = 0; __b < 6; __b++)
      ...
      for (int __b = 0; __b < 6; __b++)
        for (int __c = 0; __c < 16; __c++)
          {
            Apd[__a][__b][__c] = Get_Random < float >();
          }
    stop_timer ();
...
//=======================================================

    {
      ...
      Symmetric_Definite_Projection_SCALAR_None < data_size > op ((float *) &A,
                                                                  (float *)
                                                                  &Apd);
      ...
      for (int t = threads; t <= threads_max;
           t += std::max < int >(((threads_max - threads) / 30), 1))
        {
          ...
          MT_Streaming_Kernels::Kernel_Serial_Base_Helper <
            Symmetric_Definite_Projection_SCALAR_None < data_size > >helper (op,
                                                                             data_size,
                                                                             t);
...
//=======================================================
#ifdef ENABLE_SSE_INSTRUCTION_SET
    {
      ...
      Symmetric_Definite_Projection_SSE_None < data_size > op ((float *) &A,
                                                               (float *) &Apd);
      ...
      for (int t = threads; t <= threads_max;
           t += std::max < int >(((threads_max - threads) / 30), 1))
        {
          std::cout << "Running Test with " << t << " threads." << std::endl;
          MT_Streaming_Kernels::Kernel_Serial_Base_Helper <
            Symmetric_Definite_Projection_SSE_None < data_size > >helper (op,
                                                                          data_size,
                                                                          t);
...
//=======================================================
#ifdef ENABLE_AVX_INSTRUCTION_SET
    {
      ...
      Symmetric_Definite_Projection_AVX_None < data_size > op ((float *) &A,
                                                               (float *) &Apd);
      ...
      for (int t = threads; t <= threads_max;
           t += std::max < int >(((threads_max - threads) / 30), 1))
        {
          std::cout << "Running Test with " << t << " threads." << std::endl;
          MT_Streaming_Kernels::Kernel_Serial_Base_Helper <
            Symmetric_Definite_Projection_AVX_None < data_size > >helper (op,
                                                                          data_size,
                                                                          t);
...
//=======================================================
#ifdef ENABLE_NEON_INSTRUCTION_SET
    {
      ...
      Symmetric_Definite_Projection_NEON_None < data_size > op ((float *) &A,
                                                                (float *) &Apd);
      ...
      for (int t = threads; t <= threads_max;
           t += std::max < int >(((threads_max - threads) / 30), 1))
        {
          std::cout << "Running Test with " << t << " threads." << std::endl;
          MT_Streaming_Kernels::Kernel_Serial_Base_Helper <
            Symmetric_Definite_Projection_NEON_None < data_size > >helper (op,
                                                                           data_size,
                                                                           t);
...
//=======================================================
#ifdef ENABLE_MIC_INSTRUCTION_SET
    {
      ...
      Symmetric_Definite_Projection_MIC_None < data_size > op ((float *) &A,
                                                               (float *) &Apd);
      ...
      for (int t = threads; t <= threads_max;
           t += std::max < int >(((threads_max - threads) / 30), 1))
        {
          std::cout << "Running Test with " << t << " threads." << std::endl;
          MT_Streaming_Kernels::Kernel_Serial_Base_Helper <
            Symmetric_Definite_Projection_MIC_None < data_size > >helper (op,
                                                                          data_size,
                                                                          t);
...
//=======================================================
    ...
    _mm_free (reinterpret_cast < void *>(Apd));
