...
main (int argc, char *argv[])
{
...

  {
    ...
    for (int i = 0; i < 1; i += 1)
      {
        Unweighted_Accumulation < float, float, int >(u, F, one_over_h, scale);
      }

    Unweighted_Accumulation_Reference < float >(u_reference, F, one_over_h,
                                                scale);
    if (!(Unweighted_Accumulation_Compare < float >(u, u_reference)))
      {
        std::
          cout << "Failed to confirm unit test for Unweighted_Accumulation " <<
          std::endl;
        return 1;
      }
