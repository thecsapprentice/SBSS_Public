...
template < int SIZE > class Unweighted_Accumulation_SCALAR_None
{
private:
...
public:
    explicit Unweighted_Accumulation_SCALAR_None (float *u_in, float *F_in,
                                                  float *one_over_h_in,
                                                  float
                                                  *scale_in):_local_u (u_in),
    _local_F (F_in), _local_one_over_h (one_over_h_in), _local_scale (scale_in)
  {
  }
  void Execute (int index)
  {
    ...
    fullArray3 _rone_over_h =
      reinterpret_cast < fullArray3 > (*_local_one_over_h);
    ...
    for (int chunk_offset = 0; chunk_offset < 16; chunk_offset += ChunkSize)
      {
        ...
        refArray3 one_over_hk =
          reinterpret_cast < refArray3 > (_rone_over_h[index][chunk_offset]);
        refArray4 scalek =
...
template < int SIZE > class Unweighted_Accumulation_SSE_None
{
private:
  // Generate Variables Here
  float *_local_u;
...
public:
    explicit Unweighted_Accumulation_SSE_None (float *u_in, float *F_in,
                                               float *one_over_h_in,
...
template < int SIZE > class Unweighted_Accumulation_AVX_None
{
private:
  // Generate Variables Here
  float *_local_u;
...
public:
    explicit Unweighted_Accumulation_AVX_None (float *u_in, float *F_in,
                                               float *one_over_h_in,
                                               float *scale_in):_local_u (u_in),
    _local_F (F_in), _local_one_over_h (one_over_h_in), _local_scale (scale_in)
  {
...
template < int SIZE > class Unweighted_Accumulation_NEON_None
{
private:
  // Generate Variables Here
  float *_local_u;
...
public:
    explicit Unweighted_Accumulation_NEON_None (float *u_in, float *F_in,
                                                float *one_over_h_in,
                                                float
                                                *scale_in):_local_u (u_in),
    _local_F (F_in), _local_one_over_h (one_over_h_in), _local_scale (scale_in)
...
template < int SIZE > class Unweighted_Accumulation_MIC_None
{
private:
  // Generate Variables Here
  float *_local_u;
...
public:
    explicit Unweighted_Accumulation_MIC_None (float *u_in, float *F_in,
                                               float *one_over_h_in,
                                               float *scale_in):_local_u (u_in),
    _local_F (F_in), _local_one_over_h (one_over_h_in), _local_scale (scale_in)
  {
...
main (int argc, char *argv[])
{
  ...
  {
    std::cout << "Running Thread Test for Unweighted_Accumulation " << std::
      endl;
...
//=======================================================

    {
      ...
      Unweighted_Accumulation_SCALAR_None < data_size > op ((float *) &u,
                                                            (float *) &F,
                                                            (float *)
                                                            &one_over_h,
                                                            (float *) &scale);
      ...
      for (int t = threads; t <= threads_max;
           t += std::max < int >(((threads_max - threads) / 30), 1))
        {
          ...
          MT_Streaming_Kernels::Kernel_Serial_Base_Helper <
            Unweighted_Accumulation_SCALAR_None < data_size > >helper (op,
                                                                       data_size,
                                                                       t);
...
//=======================================================
#ifdef ENABLE_SSE_INSTRUCTION_SET
    {
      ...
      Unweighted_Accumulation_SSE_None < data_size > op ((float *) &u,
                                                         (float *) &F,
                                                         (float *) &one_over_h,
                                                         (float *) &scale);
      ...
      for (int t = threads; t <= threads_max;
           t += std::max < int >(((threads_max - threads) / 30), 1))
        {
          std::cout << "Running Test with " << t << " threads." << std::endl;
          MT_Streaming_Kernels::Kernel_Serial_Base_Helper <
            Unweighted_Accumulation_SSE_None < data_size > >helper (op,
                                                                    data_size,
                                                                    t);
...
//=======================================================
#ifdef ENABLE_AVX_INSTRUCTION_SET
    {
      ...
      Unweighted_Accumulation_AVX_None < data_size > op ((float *) &u,
                                                         (float *) &F,
                                                         (float *) &one_over_h,
                                                         (float *) &scale);
      ...
      for (int t = threads; t <= threads_max;
           t += std::max < int >(((threads_max - threads) / 30), 1))
        {
          std::cout << "Running Test with " << t << " threads." << std::endl;
          MT_Streaming_Kernels::Kernel_Serial_Base_Helper <
            Unweighted_Accumulation_AVX_None < data_size > >helper (op,
                                                                    data_size,
                                                                    t);
...
//=======================================================
#ifdef ENABLE_NEON_INSTRUCTION_SET
    {
      ...
      Unweighted_Accumulation_NEON_None < data_size > op ((float *) &u,
                                                          (float *) &F,
                                                          (float *) &one_over_h,
                                                          (float *) &scale);
      ...
      for (int t = threads; t <= threads_max;
           t += std::max < int >(((threads_max - threads) / 30), 1))
        {
          std::cout << "Running Test with " << t << " threads." << std::endl;
          MT_Streaming_Kernels::Kernel_Serial_Base_Helper <
            Unweighted_Accumulation_NEON_None < data_size > >helper (op,
                                                                     data_size,
                                                                     t);
...
//=======================================================
#ifdef ENABLE_MIC_INSTRUCTION_SET
    {
      ...
      Unweighted_Accumulation_MIC_None < data_size > op ((float *) &u,
                                                         (float *) &F,
                                                         (float *) &one_over_h,
                                                         (float *) &scale);
      ...
      for (int t = threads; t <= threads_max;
           t += std::max < int >(((threads_max - threads) / 30), 1))
        {
          std::cout << "Running Test with " << t << " threads." << std::endl;
          MT_Streaming_Kernels::Kernel_Serial_Base_Helper <
            Unweighted_Accumulation_MIC_None < data_size > >helper (op,
                                                                    data_size,
                                                                    t);
