...
#include "Weighted_Gradient_Reference.h"
...
main (int argc, char *argv[])
{
...

  {
    ...
    for (int i = 0; i < 1; i += 1)
      {
        Weighted_Gradient < float, float, int >(u, F, W, one_over_h);
      }

    Weighted_Gradient_Reference < float >(u, F_reference, W, one_over_h);
    if (!(Weighted_Gradient_Compare < float >(F, F_reference)))
      {
        std::
          cout << "Failed to confirm unit test for Weighted_Gradient " << std::
          endl;
        return 1;
      }
