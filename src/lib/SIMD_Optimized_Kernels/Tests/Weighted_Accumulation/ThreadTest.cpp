...
template < int SIZE > class Weighted_Accumulation_SCALAR_None
{
private:
...
public:
    explicit Weighted_Accumulation_SCALAR_None (float *u_in, float *F_in,
                                                float *W_in,
                                                float *one_over_h_in,
                                                float
                                                *scale_in):_local_u (u_in),
    _local_F (F_in), _local_W (W_in), _local_one_over_h (one_over_h_in),
    _local_scale (scale_in)
  {
  }
  void Execute (int index)
  {
    ...
    fullArray5 _rscale = reinterpret_cast < fullArray5 > (*_local_scale);
    ...
    for (int chunk_offset = 0; chunk_offset < 16; chunk_offset += ChunkSize)
      {
        ...
        refArray5 scalek =
          reinterpret_cast < refArray5 > (_rscale[index][chunk_offset]);

        Weighted_Accumulation < float, float[16], int[16] > (uk, Fk, Wk,
                                                             one_over_hk,
                                                             scalek);
      }
...
template < int SIZE > class Weighted_Accumulation_SSE_None
{
private:
  // Generate Variables Here
  float *_local_u;
...
public:
    explicit Weighted_Accumulation_SSE_None (float *u_in, float *F_in,
                                             float *W_in, float *one_over_h_in,
                                             float *scale_in):_local_u (u_in),
    _local_F (F_in), _local_W (W_in), _local_one_over_h (one_over_h_in),
    _local_scale (scale_in)
  {
  }
    ...
    for (int chunk_offset = 0; chunk_offset < 16; chunk_offset += ChunkSize)
      {
        ...
        Weighted_Accumulation < __m128, float[16], int[16] > (uk, Fk, Wk,
                                                              one_over_hk,
                                                              scalek);
      }
...
template < int SIZE > class Weighted_Accumulation_AVX_None
{
private:
  // Generate Variables Here
  float *_local_u;
...
public:
    explicit Weighted_Accumulation_AVX_None (float *u_in, float *F_in,
                                             float *W_in, float *one_over_h_in,
                                             float *scale_in):_local_u (u_in),
    _local_F (F_in), _local_W (W_in), _local_one_over_h (one_over_h_in),
    _local_scale (scale_in)
    ...
    for (int chunk_offset = 0; chunk_offset < 16; chunk_offset += ChunkSize)
      {
        ...
        Weighted_Accumulation < __m256, float[16], int[16] > (uk, Fk, Wk,
                                                              one_over_hk,
                                                              scalek);
      }
...
template < int SIZE > class Weighted_Accumulation_NEON_None
{
private:
  // Generate Variables Here
  float *_local_u;
...
public:
    explicit Weighted_Accumulation_NEON_None (float *u_in, float *F_in,
                                              float *W_in, float *one_over_h_in,
                                              float *scale_in):_local_u (u_in),
    _local_F (F_in), _local_W (W_in), _local_one_over_h (one_over_h_in),
    _local_scale (scale_in)
    ...
    for (int chunk_offset = 0; chunk_offset < 16; chunk_offset += ChunkSize)
      {
        ...
        Weighted_Accumulation < float32x4_t, float[16], int[16] > (uk, Fk, Wk,
                                                                   one_over_hk,
                                                                   scalek);
      }
...
template < int SIZE > class Weighted_Accumulation_MIC_None
{
private:
  // Generate Variables Here
  float *_local_u;
...
public:
    explicit Weighted_Accumulation_MIC_None (float *u_in, float *F_in,
                                             float *W_in, float *one_over_h_in,
                                             float *scale_in):_local_u (u_in),
    _local_F (F_in), _local_W (W_in), _local_one_over_h (one_over_h_in),
    _local_scale (scale_in)
    ...
    for (int chunk_offset = 0; chunk_offset < 16; chunk_offset += ChunkSize)
      {
        ...
        Weighted_Accumulation < __m512, float[16], int[16] > (uk, Fk, Wk,
                                                              one_over_hk,
                                                              scalek);
      }
...
main (int argc, char *argv[])
{
  ...
  {
    std::cout << "Running Thread Test for Weighted_Accumulation " << std::endl;
...
//=======================================================

    {
      ...
      Weighted_Accumulation_SCALAR_None < data_size > op ((float *) &u,
                                                          (float *) &F,
                                                          (float *) &W,
                                                          (float *) &one_over_h,
                                                          (float *) &scale);
      ...
      for (int t = threads; t <= threads_max;
           t += std::max < int >(((threads_max - threads) / 30), 1))
        {
          ...
          MT_Streaming_Kernels::Kernel_Serial_Base_Helper <
            Weighted_Accumulation_SCALAR_None < data_size > >helper (op,
                                                                     data_size,
                                                                     t);
...
//=======================================================
#ifdef ENABLE_SSE_INSTRUCTION_SET
    {
      ...
      Weighted_Accumulation_SSE_None < data_size > op ((float *) &u,
                                                       (float *) &F,
                                                       (float *) &W,
                                                       (float *) &one_over_h,
                                                       (float *) &scale);
      ...
      for (int t = threads; t <= threads_max;
           t += std::max < int >(((threads_max - threads) / 30), 1))
        {
          std::cout << "Running Test with " << t << " threads." << std::endl;
          MT_Streaming_Kernels::Kernel_Serial_Base_Helper <
            Weighted_Accumulation_SSE_None < data_size > >helper (op, data_size,
                                                                  t);
...
//=======================================================
#ifdef ENABLE_AVX_INSTRUCTION_SET
    {
      ...
      Weighted_Accumulation_AVX_None < data_size > op ((float *) &u,
                                                       (float *) &F,
                                                       (float *) &W,
                                                       (float *) &one_over_h,
                                                       (float *) &scale);
      ...
      for (int t = threads; t <= threads_max;
           t += std::max < int >(((threads_max - threads) / 30), 1))
        {
          std::cout << "Running Test with " << t << " threads." << std::endl;
          MT_Streaming_Kernels::Kernel_Serial_Base_Helper <
            Weighted_Accumulation_AVX_None < data_size > >helper (op, data_size,
                                                                  t);
...
//=======================================================
#ifdef ENABLE_NEON_INSTRUCTION_SET
    {
      ...
      Weighted_Accumulation_NEON_None < data_size > op ((float *) &u,
                                                        (float *) &F,
                                                        (float *) &W,
                                                        (float *) &one_over_h,
                                                        (float *) &scale);
      ...
      for (int t = threads; t <= threads_max;
           t += std::max < int >(((threads_max - threads) / 30), 1))
        {
          std::cout << "Running Test with " << t << " threads." << std::endl;
          MT_Streaming_Kernels::Kernel_Serial_Base_Helper <
            Weighted_Accumulation_NEON_None < data_size > >helper (op,
                                                                   data_size,
                                                                   t);
...
//=======================================================
#ifdef ENABLE_MIC_INSTRUCTION_SET
    {
      ...
      Weighted_Accumulation_MIC_None < data_size > op ((float *) &u,
                                                       (float *) &F,
                                                       (float *) &W,
                                                       (float *) &one_over_h,
                                                       (float *) &scale);
      ...
      for (int t = threads; t <= threads_max;
           t += std::max < int >(((threads_max - threads) / 30), 1))
        {
          std::cout << "Running Test with " << t << " threads." << std::endl;
          MT_Streaming_Kernels::Kernel_Serial_Base_Helper <
            Weighted_Accumulation_MIC_None < data_size > >helper (op, data_size,
                                                                  t);
