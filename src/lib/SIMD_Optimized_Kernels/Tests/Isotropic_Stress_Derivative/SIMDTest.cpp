...
main (int argc, char *argv[])
{
  ...
  {
    std::cout << "Running SIMD Test for Isotropic_Stress_Derivative " << std::
      endl;
...
//=======================================================
    ...
    bool __mapply_definiteness_fix __attribute__ ((aligned (4)));
    for (int k = 0; k < 16; k++)
    {
      for (int __a = 0; __a < 12; __a++)
      ...
      __mkappa = kappa[k];
      __malpha = alpha[k];
      __mapply_definiteness_fix = apply_definiteness_fix;
      Isotropic_Stress_Derivative < NEOHOOKEAN_TAG, float, float,
        int >::Run (__mdPdF_reference, __mSigma, __mp, __mmu, __mkappa,
                    __malpha, __mapply_definiteness_fix);
      for (int __a = 0; __a < 12; __a++)
