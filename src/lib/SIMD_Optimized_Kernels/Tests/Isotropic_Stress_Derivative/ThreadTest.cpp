...
template < int SIZE > class Isotropic_Stress_Derivative_SCALAR_NEOHOOKEAN
{
private:
  ...
  float *_local_kappa;
  float *_local_alpha;
  bool *_local_apply_definiteness_fix;

public:
    explicit Isotropic_Stress_Derivative_SCALAR_NEOHOOKEAN (float *dPdF_in,
                                                            float *Sigma_in,
                                                            float *p_in,
                                                            float *mu_in,
                                                            float *kappa_in,
                                                            float *alpha_in,
                                                            bool *
                                                            apply_definiteness_fix_in):_local_dPdF
    (dPdF_in), _local_Sigma (Sigma_in), _local_p (p_in), _local_mu (mu_in),
    _local_kappa (kappa_in), _local_alpha (alpha_in),
    _local_apply_definiteness_fix (apply_definiteness_fix_in)
  {
  }
  void Execute (int index)
  {
    ...
    fullArray5 _rkappa = reinterpret_cast < fullArray5 > (*_local_kappa);
    fullArray6 _ralpha = reinterpret_cast < fullArray6 > (*_local_alpha);
    fullArray7 _rapply_definiteness_fix =
      reinterpret_cast < fullArray7 > (*_local_apply_definiteness_fix);
    ...
    for (int chunk_offset = 0; chunk_offset < 16; chunk_offset += ChunkSize)
      {
        ...
        refArray5 kappak =
          reinterpret_cast < refArray5 > (_rkappa[index][chunk_offset]);
        refArray6 alphak =
          reinterpret_cast < refArray6 > (_ralpha[index][chunk_offset]);
        refArray7 apply_definiteness_fixk =
          reinterpret_cast < refArray7 > (_rapply_definiteness_fix[index]);
...
template < int SIZE > class Isotropic_Stress_Derivative_SCALAR_COROTATED
{
private:
  // Generate Variables Here
  float *_local_dPdF;
...
public:
    explicit Isotropic_Stress_Derivative_SCALAR_COROTATED (float *dPdF_in,
                                                           float *Sigma_in,
                                                           float *p_in,
                                                           float *mu_in,
                                                           float *kappa_in,
...
template < int SIZE > class Isotropic_Stress_Derivative_SSE_NEOHOOKEAN
{
private:
  // Generate Variables Here
  float *_local_dPdF;
...
public:
    explicit Isotropic_Stress_Derivative_SSE_NEOHOOKEAN (float *dPdF_in,
                                                         float *Sigma_in,
                                                         float *p_in,
                                                         float *mu_in,
                                                         float *kappa_in,
...
template < int SIZE > class Isotropic_Stress_Derivative_SSE_COROTATED
{
private:
  // Generate Variables Here
  float *_local_dPdF;
...
public:
    explicit Isotropic_Stress_Derivative_SSE_COROTATED (float *dPdF_in,
                                                        float *Sigma_in,
                                                        float *p_in,
                                                        float *mu_in,
                                                        float *kappa_in,
...
template < int SIZE > class Isotropic_Stress_Derivative_AVX_NEOHOOKEAN
{
private:
  // Generate Variables Here
  float *_local_dPdF;
...
public:
    explicit Isotropic_Stress_Derivative_AVX_NEOHOOKEAN (float *dPdF_in,
                                                         float *Sigma_in,
                                                         float *p_in,
                                                         float *mu_in,
                                                         float *kappa_in,
...
template < int SIZE > class Isotropic_Stress_Derivative_AVX_COROTATED
{
private:
  // Generate Variables Here
  float *_local_dPdF;
...
public:
    explicit Isotropic_Stress_Derivative_AVX_COROTATED (float *dPdF_in,
                                                        float *Sigma_in,
                                                        float *p_in,
                                                        float *mu_in,
                                                        float *kappa_in,
...
template < int SIZE > class Isotropic_Stress_Derivative_NEON_NEOHOOKEAN
{
private:
  // Generate Variables Here
  float *_local_dPdF;
...
public:
    explicit Isotropic_Stress_Derivative_NEON_NEOHOOKEAN (float *dPdF_in,
                                                          float *Sigma_in,
                                                          float *p_in,
                                                          float *mu_in,
                                                          float *kappa_in,
    ...
    for (int chunk_offset = 0; chunk_offset < 16; chunk_offset += ChunkSize)
      {
        ...
        Isotropic_Stress_Derivative < NEOHOOKEAN_TAG, float32x4_t, float[16],
          int[16] >::Run (dPdFk, Sigmak, pk, muk, kappak, alphak,
                          apply_definiteness_fixk);
      }
...
template < int SIZE > class Isotropic_Stress_Derivative_NEON_COROTATED
{
private:
  // Generate Variables Here
  float *_local_dPdF;
...
public:
    explicit Isotropic_Stress_Derivative_NEON_COROTATED (float *dPdF_in,
                                                         float *Sigma_in,
                                                         float *p_in,
                                                         float *mu_in,
                                                         float *kappa_in,
    ...
    for (int chunk_offset = 0; chunk_offset < 16; chunk_offset += ChunkSize)
      {
        ...
        Isotropic_Stress_Derivative < COROTATED_TAG, float32x4_t, float[16],
          int[16] >::Run (dPdFk, Sigmak, pk, muk, kappak, alphak,
                          apply_definiteness_fixk);
      }
...
template < int SIZE > class Isotropic_Stress_Derivative_MIC_NEOHOOKEAN
{
private:
  // Generate Variables Here
  float *_local_dPdF;
...
public:
    explicit Isotropic_Stress_Derivative_MIC_NEOHOOKEAN (float *dPdF_in,
                                                         float *Sigma_in,
                                                         float *p_in,
                                                         float *mu_in,
                                                         float *kappa_in,
...
template < int SIZE > class Isotropic_Stress_Derivative_MIC_COROTATED
{
private:
  // Generate Variables Here
  float *_local_dPdF;
...
public:
    explicit Isotropic_Stress_Derivative_MIC_COROTATED (float *dPdF_in,
                                                        float *Sigma_in,
                                                        float *p_in,
                                                        float *mu_in,
                                                        float *kappa_in,
...
main (int argc, char *argv[])
{
  ...
  {
    std::cout << "Running Thread Test for Isotropic_Stress_Derivative " << std::
      endl;
...
//=======================================================
    ...
    alpha_type alpha =
      ...
      (*((T *) (_mm_malloc (data_size * 16 * sizeof (T), 64))));
    typedef bool (&apply_definiteness_fix_type)[data_size];
    apply_definiteness_fix_type apply_definiteness_fix =
      reinterpret_cast < apply_definiteness_fix_type >
      (*((T *) (_mm_malloc (data_size * sizeof (bool), 64))));
    ...
    for (int __a = 0; __a < data_size; __a++)
      ...
      {
        apply_definiteness_fix[__a] = true;
      }
    stop_timer ();
...
//=======================================================

    {
      ...
      Isotropic_Stress_Derivative_SCALAR_NEOHOOKEAN < data_size >
        op ((float *) &dPdF, (float *) &Sigma, (float *) &p, (float *) &mu,
            (float *) &kappa, (float *) &alpha,
            (bool *) & apply_definiteness_fix);
...
//=======================================================
#ifdef ENABLE_SSE_INSTRUCTION_SET
    {
      ...
      Isotropic_Stress_Derivative_SSE_NEOHOOKEAN < data_size >
        op ((float *) &dPdF, (float *) &Sigma, (float *) &p, (float *) &mu,
            (float *) &kappa, (float *) &alpha,
            (bool *) & apply_definiteness_fix);
...
//=======================================================
#ifdef ENABLE_AVX_INSTRUCTION_SET
    {
      ...
      Isotropic_Stress_Derivative_AVX_NEOHOOKEAN < data_size >
        op ((float *) &dPdF, (float *) &Sigma, (float *) &p, (float *) &mu,
            (float *) &kappa, (float *) &alpha,
            (bool *) & apply_definiteness_fix);
...
//=======================================================
#ifdef ENABLE_NEON_INSTRUCTION_SET
    {
      ...
      Isotropic_Stress_Derivative_NEON_NEOHOOKEAN < data_size >
        op ((float *) &dPdF, (float *) &Sigma, (float *) &p, (float *) &mu,
            (float *) &kappa, (float *) &alpha,
            (bool *) & apply_definiteness_fix);
...
//=======================================================
#ifdef ENABLE_MIC_INSTRUCTION_SET
    {
      ...
      Isotropic_Stress_Derivative_MIC_NEOHOOKEAN < data_size >
        op ((float *) &dPdF, (float *) &Sigma, (float *) &p, (float *) &mu,
            (float *) &kappa, (float *) &alpha,
            (bool *) & apply_definiteness_fix);
...
//=======================================================
    ...
    _mm_free (reinterpret_cast < void *>(apply_definiteness_fix));
...
//=======================================================

    {
      ...
      Isotropic_Stress_Derivative_SCALAR_COROTATED < data_size >
        op ((float *) &dPdF, (float *) &Sigma, (float *) &p, (float *) &mu,
            (float *) &kappa, (float *) &alpha,
            (bool *) & apply_definiteness_fix);
...
//=======================================================
#ifdef ENABLE_SSE_INSTRUCTION_SET
    {
      ...
      Isotropic_Stress_Derivative_SSE_COROTATED < data_size >
        op ((float *) &dPdF, (float *) &Sigma, (float *) &p, (float *) &mu,
            (float *) &kappa, (float *) &alpha,
            (bool *) & apply_definiteness_fix);
      ...
      for (int t = threads; t <= threads_max;
           t += std::max < int >(((threads_max - threads) / 30), 1))
        {
          std::cout << "Running Test with " << t << " threads." << std::endl;
          MT_Streaming_Kernels::Kernel_Serial_Base_Helper <
            Isotropic_Stress_Derivative_SSE_COROTATED < data_size > >helper (op,
                                                                             data_size,
                                                                             t);
...
//=======================================================
#ifdef ENABLE_AVX_INSTRUCTION_SET
    {
      ...
      Isotropic_Stress_Derivative_AVX_COROTATED < data_size >
        op ((float *) &dPdF, (float *) &Sigma, (float *) &p, (float *) &mu,
            (float *) &kappa, (float *) &alpha,
            (bool *) & apply_definiteness_fix);
      ...
      for (int t = threads; t <= threads_max;
           t += std::max < int >(((threads_max - threads) / 30), 1))
        {
          std::cout << "Running Test with " << t << " threads." << std::endl;
          MT_Streaming_Kernels::Kernel_Serial_Base_Helper <
            Isotropic_Stress_Derivative_AVX_COROTATED < data_size > >helper (op,
                                                                             data_size,
                                                                             t);
...
//=======================================================
#ifdef ENABLE_NEON_INSTRUCTION_SET
    {
      ...
      Isotropic_Stress_Derivative_NEON_COROTATED < data_size >
        op ((float *) &dPdF, (float *) &Sigma, (float *) &p, (float *) &mu,
            (float *) &kappa, (float *) &alpha,
            (bool *) & apply_definiteness_fix);
...
//=======================================================
#ifdef ENABLE_MIC_INSTRUCTION_SET
    {
      ...
      Isotropic_Stress_Derivative_MIC_COROTATED < data_size >
        op ((float *) &dPdF, (float *) &Sigma, (float *) &p, (float *) &mu,
            (float *) &kappa, (float *) &alpha,
            (bool *) & apply_definiteness_fix);
      ...
      for (int t = threads; t <= threads_max;
           t += std::max < int >(((threads_max - threads) / 30), 1))
        {
          std::cout << "Running Test with " << t << " threads." << std::endl;
          MT_Streaming_Kernels::Kernel_Serial_Base_Helper <
            Isotropic_Stress_Derivative_MIC_COROTATED < data_size > >helper (op,
                                                                             data_size,
                                                                             t);
