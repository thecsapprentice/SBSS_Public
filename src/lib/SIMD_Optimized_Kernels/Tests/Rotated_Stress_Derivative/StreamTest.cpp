...
main (int argc, char *argv[])
{
  ...
  {
    std::cout << "Running Stream Test for Rotated_Stress_Derivative " << std::
      endl;
          ...
          for (int i = 0; i < 16; i += 4)
            {
              ...
              refArray3 muk = reinterpret_cast < refArray3 > (mu[i]);
              refArray4 kappak = reinterpret_cast < refArray4 > (kappa[i]);
              Rotated_Stress_Derivative < NEOHOOKEAN_TAG, float32x4_t,
                float[16], int[16] >::Run (dPdFk, Sigmak, muk, kappak);
            }
