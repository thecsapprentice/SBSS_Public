...
template < int SIZE > class Muscle_Differential_SCALAR_None
{
private:
  // Generate Variables Here
  float *_local_dP_fiber;
  float *_local_dF;
  float *_local_fiber;
  float *_local_Ffiber;
  float *_local_c1;
...
public:
    explicit Muscle_Differential_SCALAR_None (float *dP_fiber_in, float *dF_in,
                                              float *fiber_in, float *Ffiber_in,
                                              float *c1_in,
                                              float
                                              *c2_in):_local_dP_fiber
    (dP_fiber_in), _local_dF (dF_in), _local_fiber (fiber_in),
    _local_Ffiber (Ffiber_in), _local_c1 (c1_in), _local_c2 (c2_in)
  {
  }
  void Execute (int index)
  {
    ...
    fullArray1 _rdP_fiber = reinterpret_cast < fullArray1 > (*_local_dP_fiber);
    fullArray2 _rdF = reinterpret_cast < fullArray2 > (*_local_dF);
    fullArray3 _rfiber = reinterpret_cast < fullArray3 > (*_local_fiber);
    fullArray4 _rFfiber = reinterpret_cast < fullArray4 > (*_local_Ffiber);
    fullArray5 _rc1 = reinterpret_cast < fullArray5 > (*_local_c1);
    fullArray6 _rc2 = reinterpret_cast < fullArray6 > (*_local_c2);
    ...
    for (int chunk_offset = 0; chunk_offset < 16; chunk_offset += ChunkSize)
      {
        ...
        refArray1 dP_fiberk =
          reinterpret_cast < refArray1 > (_rdP_fiber[index][0][chunk_offset]);
        refArray2 dFk =
          reinterpret_cast < refArray2 > (_rdF[index][0][chunk_offset]);
        refArray3 fiberk =
          reinterpret_cast < refArray3 > (_rfiber[index][0][chunk_offset]);
        refArray4 Ffiberk =
          reinterpret_cast < refArray4 > (_rFfiber[index][0][chunk_offset]);
        refArray5 c1k =
          reinterpret_cast < refArray5 > (_rc1[index][chunk_offset]);
        refArray6 c2k =
          reinterpret_cast < refArray6 > (_rc2[index][chunk_offset]);
...
template < int SIZE > class Muscle_Differential_SSE_None
{
private:
  // Generate Variables Here
  float *_local_dP_fiber;
...
public:
    explicit Muscle_Differential_SSE_None (float *dP_fiber_in, float *dF_in,
                                           float *fiber_in, float *Ffiber_in,
                                           float *c1_in,
                                           float
                                           *c2_in):_local_dP_fiber
...
template < int SIZE > class Muscle_Differential_AVX_None
{
private:
  // Generate Variables Here
  float *_local_dP_fiber;
...
public:
    explicit Muscle_Differential_AVX_None (float *dP_fiber_in, float *dF_in,
                                           float *fiber_in, float *Ffiber_in,
                                           float *c1_in,
                                           float
                                           *c2_in):_local_dP_fiber
...
template < int SIZE > class Muscle_Differential_NEON_None
{
private:
  // Generate Variables Here
  float *_local_dP_fiber;
...
public:
    explicit Muscle_Differential_NEON_None (float *dP_fiber_in, float *dF_in,
                                            float *fiber_in, float *Ffiber_in,
                                            float *c1_in,
                                            float
                                            *c2_in):_local_dP_fiber
...
template < int SIZE > class Muscle_Differential_MIC_None
{
private:
  // Generate Variables Here
  float *_local_dP_fiber;
...
public:
    explicit Muscle_Differential_MIC_None (float *dP_fiber_in, float *dF_in,
                                           float *fiber_in, float *Ffiber_in,
                                           float *c1_in,
                                           float
                                           *c2_in):_local_dP_fiber
...
main (int argc, char *argv[])
{
  ...
  {
    std::cout << "Running Thread Test for Muscle_Differential " << std::endl;
...
//=======================================================
    ...
    start_timer ();
    typedef T (&dP_fiber_type)[data_size][9][16];
    dP_fiber_type dP_fiber =
      reinterpret_cast < dP_fiber_type >
      (*((T *) (_mm_malloc (data_size * 9 * 16 * sizeof (T), 64))));
    typedef T (&dF_type)[data_size][9][16];
    dF_type dF =
    ...
    for (int __a = 0; __a < data_size; __a++)
      for (int __b = 0; __b < 9; __b++)
        for (int __c = 0; __c < 16; __c++)
          {
            dP_fiber[__a][__b][__c] = Get_Random < float >();
    } for (int __a = 0; __a < data_size; __a++)
      for (int __b = 0; __b < 9; __b++)
        for (int __c = 0; __c < 16; __c++)
          {
            dF[__a][__b][__c] = Get_Random < float >();
    } for (int __a = 0; __a < data_size; __a++)
...
//=======================================================

    {
      ...
      Muscle_Differential_SCALAR_None < data_size > op ((float *) &dP_fiber,
                                                        (float *) &dF,
                                                        (float *) &fiber,
                                                        (float *) &Ffiber,
                                                        (float *) &c1,
      ...
      for (int t = threads; t <= threads_max;
           t += std::max < int >(((threads_max - threads) / 30), 1))
        {
          ...
          MT_Streaming_Kernels::Kernel_Serial_Base_Helper <
            Muscle_Differential_SCALAR_None < data_size > >helper (op,
                                                                   data_size,
                                                                   t);
...
//=======================================================
#ifdef ENABLE_SSE_INSTRUCTION_SET
    {
      ...
      Muscle_Differential_SSE_None < data_size > op ((float *) &dP_fiber,
                                                     (float *) &dF,
                                                     (float *) &fiber,
                                                     (float *) &Ffiber,
                                                     (float *) &c1,
      ...
      for (int t = threads; t <= threads_max;
           t += std::max < int >(((threads_max - threads) / 30), 1))
        {
          std::cout << "Running Test with " << t << " threads." << std::endl;
          MT_Streaming_Kernels::Kernel_Serial_Base_Helper <
            Muscle_Differential_SSE_None < data_size > >helper (op, data_size,
                                                                t);
...
//=======================================================
#ifdef ENABLE_AVX_INSTRUCTION_SET
    {
      ...
      Muscle_Differential_AVX_None < data_size > op ((float *) &dP_fiber,
                                                     (float *) &dF,
                                                     (float *) &fiber,
                                                     (float *) &Ffiber,
                                                     (float *) &c1,
      ...
      for (int t = threads; t <= threads_max;
           t += std::max < int >(((threads_max - threads) / 30), 1))
        {
          std::cout << "Running Test with " << t << " threads." << std::endl;
          MT_Streaming_Kernels::Kernel_Serial_Base_Helper <
            Muscle_Differential_AVX_None < data_size > >helper (op, data_size,
                                                                t);
...
//=======================================================
#ifdef ENABLE_NEON_INSTRUCTION_SET
    {
      ...
      Muscle_Differential_NEON_None < data_size > op ((float *) &dP_fiber,
                                                      (float *) &dF,
                                                      (float *) &fiber,
                                                      (float *) &Ffiber,
                                                      (float *) &c1,
      ...
      for (int t = threads; t <= threads_max;
           t += std::max < int >(((threads_max - threads) / 30), 1))
        {
          std::cout << "Running Test with " << t << " threads." << std::endl;
          MT_Streaming_Kernels::Kernel_Serial_Base_Helper <
            Muscle_Differential_NEON_None < data_size > >helper (op, data_size,
                                                                 t);
...
//=======================================================
#ifdef ENABLE_MIC_INSTRUCTION_SET
    {
      ...
      Muscle_Differential_MIC_None < data_size > op ((float *) &dP_fiber,
                                                     (float *) &dF,
                                                     (float *) &fiber,
                                                     (float *) &Ffiber,
                                                     (float *) &c1,
      ...
      for (int t = threads; t <= threads_max;
           t += std::max < int >(((threads_max - threads) / 30), 1))
        {
          std::cout << "Running Test with " << t << " threads." << std::endl;
          MT_Streaming_Kernels::Kernel_Serial_Base_Helper <
            Muscle_Differential_MIC_None < data_size > >helper (op, data_size,
                                                                t);
...
//=======================================================
    ...
    _mm_free (reinterpret_cast < void *>(dP_fiber));
