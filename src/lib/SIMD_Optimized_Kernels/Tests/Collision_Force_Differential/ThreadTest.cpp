...
template < int SIZE > class Collision_Force_Differential_SCALAR_None
{
private:
...
public:
    explicit Collision_Force_Differential_SCALAR_None (float *f_in, float *u_in,
                                                       float *W_in,
                                                       int *spring_id_in,
                                                       float
                                                       **extern_collision_stiffness_in):_local_f
    (f_in), _local_u (u_in), _local_W (W_in), _local_spring_id (spring_id_in),
  ...
  void Execute (int index)
  {
    ...
    typedef int (&fullArray4)[SIZE][16];
    typedef float *(&fullArray5)[SIZE];
    ...
    fullArray4 _rspring_id =
      reinterpret_cast < fullArray4 > (*_local_spring_id);
    fullArray5 _rextern_collision_stiffness =
      reinterpret_cast < fullArray5 > (*_local_extern_collision_stiffness);
    ...
    for (int chunk_offset = 0; chunk_offset < 16; chunk_offset += ChunkSize)
      {
        ...
        refArray4 spring_idk =
          reinterpret_cast < refArray4 > (_rspring_id[index][chunk_offset]);
        refArray5 extern_collision_stiffnessk =
          reinterpret_cast < refArray5 > (_rextern_collision_stiffness[index]);

        Collision_Force_Differential < float, float[16], int[16] > (fk, uk, Wk,
                                                                    spring_idk,
...
template < int SIZE > class Collision_Force_Differential_SSE_None
{
private:
  // Generate Variables Here
  float *_local_f;
...
public:
    explicit Collision_Force_Differential_SSE_None (float *f_in, float *u_in,
                                                    float *W_in,
                                                    int *spring_id_in,
                                                    float
                                                    **extern_collision_stiffness_in):_local_f
    ...
    for (int chunk_offset = 0; chunk_offset < 16; chunk_offset += ChunkSize)
      {
        ...
        Collision_Force_Differential < __m128, float[16], int[16] > (fk, uk, Wk,
                                                                     spring_idk,
                                                                     extern_collision_stiffnessk);
      }
...
template < int SIZE > class Collision_Force_Differential_AVX_None
{
private:
  // Generate Variables Here
  float *_local_f;
...
public:
    explicit Collision_Force_Differential_AVX_None (float *f_in, float *u_in,
                                                    float *W_in,
                                                    int *spring_id_in,
                                                    float
                                                    **extern_collision_stiffness_in):_local_f
    ...
    for (int chunk_offset = 0; chunk_offset < 16; chunk_offset += ChunkSize)
      {
        ...
        Collision_Force_Differential < __m256, float[16], int[16] > (fk, uk, Wk,
                                                                     spring_idk,
                                                                     extern_collision_stiffnessk);
      }
...
template < int SIZE > class Collision_Force_Differential_NEON_None
{
private:
  // Generate Variables Here
  float *_local_f;
...
public:
    explicit Collision_Force_Differential_NEON_None (float *f_in, float *u_in,
                                                     float *W_in,
                                                     int *spring_id_in,
                                                     float
                                                     **extern_collision_stiffness_in):_local_f
    ...
    for (int chunk_offset = 0; chunk_offset < 16; chunk_offset += ChunkSize)
      {
        ...
        Collision_Force_Differential < float32x4_t, float[16], int[16] > (fk,
                                                                          uk,
                                                                          Wk,
                                                                          spring_idk,
                                                                          extern_collision_stiffnessk);
...
template < int SIZE > class Collision_Force_Differential_MIC_None
{
private:
  // Generate Variables Here
  float *_local_f;
...
public:
    explicit Collision_Force_Differential_MIC_None (float *f_in, float *u_in,
                                                    float *W_in,
                                                    int *spring_id_in,
                                                    float
                                                    **extern_collision_stiffness_in):_local_f
    ...
    for (int chunk_offset = 0; chunk_offset < 16; chunk_offset += ChunkSize)
      {
        ...
        Collision_Force_Differential < __m512, float[16], int[16] > (fk, uk, Wk,
                                                                     spring_idk,
                                                                     extern_collision_stiffnessk);
      }
...
main (int argc, char *argv[])
{
  ...
  {
    std::
      cout << "Running Thread Test for Collision_Force_Differential " << std::
      endl;
...
//=======================================================

    {
      ...
      Collision_Force_Differential_SCALAR_None < data_size > op ((float *) &f,
                                                                 (float *) &u,
                                                                 (float *) &W,
                                                                 (int *)
                                                                 &spring_id,
      ...
      for (int t = threads; t <= threads_max;
           t += std::max < int >(((threads_max - threads) / 30), 1))
        {
          ...
          MT_Streaming_Kernels::Kernel_Serial_Base_Helper <
            Collision_Force_Differential_SCALAR_None < data_size > >helper (op,
                                                                            data_size,
                                                                            t);
...
//=======================================================
#ifdef ENABLE_SSE_INSTRUCTION_SET
    {
      ...
      Collision_Force_Differential_SSE_None < data_size > op ((float *) &f,
                                                              (float *) &u,
                                                              (float *) &W,
                                                              (int *)
                                                              &spring_id,
      ...
      for (int t = threads; t <= threads_max;
           t += std::max < int >(((threads_max - threads) / 30), 1))
        {
          std::cout << "Running Test with " << t << " threads." << std::endl;
          MT_Streaming_Kernels::Kernel_Serial_Base_Helper <
            Collision_Force_Differential_SSE_None < data_size > >helper (op,
                                                                         data_size,
                                                                         t);
...
//=======================================================
#ifdef ENABLE_AVX_INSTRUCTION_SET
    {
      ...
      Collision_Force_Differential_AVX_None < data_size > op ((float *) &f,
                                                              (float *) &u,
                                                              (float *) &W,
                                                              (int *)
                                                              &spring_id,
      ...
      for (int t = threads; t <= threads_max;
           t += std::max < int >(((threads_max - threads) / 30), 1))
        {
          std::cout << "Running Test with " << t << " threads." << std::endl;
          MT_Streaming_Kernels::Kernel_Serial_Base_Helper <
            Collision_Force_Differential_AVX_None < data_size > >helper (op,
                                                                         data_size,
                                                                         t);
...
//=======================================================
#ifdef ENABLE_NEON_INSTRUCTION_SET
    {
      ...
      Collision_Force_Differential_NEON_None < data_size > op ((float *) &f,
                                                               (float *) &u,
                                                               (float *) &W,
                                                               (int *)
                                                               &spring_id,
      ...
      for (int t = threads; t <= threads_max;
           t += std::max < int >(((threads_max - threads) / 30), 1))
        {
          std::cout << "Running Test with " << t << " threads." << std::endl;
          MT_Streaming_Kernels::Kernel_Serial_Base_Helper <
            Collision_Force_Differential_NEON_None < data_size > >helper (op,
                                                                          data_size,
                                                                          t);
...
//=======================================================
#ifdef ENABLE_MIC_INSTRUCTION_SET
    {
      ...
      Collision_Force_Differential_MIC_None < data_size > op ((float *) &f,
                                                              (float *) &u,
                                                              (float *) &W,
                                                              (int *)
                                                              &spring_id,
      ...
      for (int t = threads; t <= threads_max;
           t += std::max < int >(((threads_max - threads) / 30), 1))
        {
          std::cout << "Running Test with " << t << " threads." << std::endl;
          MT_Streaming_Kernels::Kernel_Serial_Base_Helper <
            Collision_Force_Differential_MIC_None < data_size > >helper (op,
                                                                         data_size,
                                                                         t);
