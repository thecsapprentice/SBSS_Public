...
main (int argc, char *argv[])
{
...

  {
    ...
    for (int i = 0; i < 1; i += 1)
      {
        Matrix_Times_Transpose < float, float, int >(A, B, C);
      }

    Matrix_Times_Transpose_Reference < float >(A, B, C_reference);
    if (!(Matrix_Times_Transpose_Compare < float >(C, C_reference)))
      {
        std::
