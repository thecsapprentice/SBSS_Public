...
template < int SIZE > class Matrix_Times_Transpose_SSE_None
{
private:
  // Generate Variables Here
  float *_local_A;
...
public:
    explicit Matrix_Times_Transpose_SSE_None (float *A_in, float *B_in,
                                              float *C_in):_local_A (A_in),
    _local_B (B_in), _local_C (C_in)
  {
  }
...
template < int SIZE > class Matrix_Times_Transpose_NEON_None
{
private:
  // Generate Variables Here
  float *_local_A;
...
public:
    explicit Matrix_Times_Transpose_NEON_None (float *A_in, float *B_in,
                                               float *C_in):_local_A (A_in),
    _local_B (B_in), _local_C (C_in)
  {
  }
...
//=======================================================
#ifdef ENABLE_SSE_INSTRUCTION_SET
    {
      ...
      Matrix_Times_Transpose_SSE_None < data_size > op ((float *) &A,
                                                        (float *) &B,
                                                        (float *) &C);
      ...
      for (int t = threads; t <= threads_max;
           t += std::max < int >(((threads_max - threads) / 30), 1))
        {
          std::cout << "Running Test with " << t << " threads." << std::endl;
          MT_Streaming_Kernels::Kernel_Serial_Base_Helper <
            Matrix_Times_Transpose_SSE_None < data_size > >helper (op,
                                                                   data_size,
                                                                   t);
...
//=======================================================
#ifdef ENABLE_NEON_INSTRUCTION_SET
    {
      ...
      Matrix_Times_Transpose_NEON_None < data_size > op ((float *) &A,
                                                         (float *) &B,
                                                         (float *) &C);
      ...
      for (int t = threads; t <= threads_max;
           t += std::max < int >(((threads_max - threads) / 30), 1))
        {
          std::cout << "Running Test with " << t << " threads." << std::endl;
          MT_Streaming_Kernels::Kernel_Serial_Base_Helper <
            Matrix_Times_Transpose_NEON_None < data_size > >helper (op,
                                                                    data_size,
                                                                    t);
