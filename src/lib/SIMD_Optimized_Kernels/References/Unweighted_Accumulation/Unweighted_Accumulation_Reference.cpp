...
{
    MATRIX_MXN<T> G;
    const VECTOR<T,3> weights=VECTOR<T,3>::All_Ones_Vector()*(T).5;
...
template void Unweighted_Accumulation_Reference(float u[3][8], const float F[9], const float one_over_h, const float scale);
template bool Unweighted_Accumulation_Compare(const float u[3][8], const float u_reference[3][8]);
