...
void Augmented_Rotated_Stress_Derivative_Corotated_Reference(T dPdF[12], const T Sigma[3], const T p, const T mu, const T alpha)
{
    ...
    rdPdF=COROTATED<T,3>::Augmented_Rotated_Stress_Derivative(mSigma,p,mu,alpha);
}
...
template void Augmented_Rotated_Stress_Derivative_Corotated_Reference(float dPdF[12], const float Sigma[3], const float p, const float mu, const float alpha);
template bool Augmented_Rotated_Stress_Derivative_Corotated_Compare(const float dPdF[12], const float dPdF_reference[12]);
...
void Augmented_Rotated_Stress_Derivative_Neohookean_Reference(T dPdF[12], const T Sigma[3], const T p, const T mu, const T alpha)
{
    ROTATED_STRESS_DERIVATIVE<T,3>& rdPdF=*(ROTATED_STRESS_DERIVATIVE<T,3>*)(dPdF);
    const DIAGONAL_MATRIX<T,3>& mSigma=*(const DIAGONAL_MATRIX<T,3>*)(Sigma);    
    rdPdF=NEOHOOKEAN<T,3>::Augmented_Rotated_Stress_Derivative(mSigma,p,mu,alpha);
}
...
template void Augmented_Rotated_Stress_Derivative_Neohookean_Reference(float dPdF[12], const float Sigma[3], const float p, const float mu, const float alpha);
template bool Augmented_Rotated_Stress_Derivative_Neohookean_Compare(const float dPdF[12], const float dPdF_reference[12]);
