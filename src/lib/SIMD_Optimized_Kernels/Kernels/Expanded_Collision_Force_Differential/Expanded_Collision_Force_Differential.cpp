...
#ifndef SUBROUTINE_Expanded_Collision_Force_Differential
#include <assert.h>
...
#ifdef SUBROUTINE_Expanded_Collision_Force_Differential
inline
#endif
void Expanded_Collision_Force_Differential(T_DATA (&f)[3][8],
                      const T_DATA (&u)[3][8],
                      const T_DATA (&W)[3],
                      const I_DATA (&flag),
                      const T_DATA (&spring_constant),  
                      ...
                      const float* extern_collision_stiffness )
{
    ...
    // On a per channel basis, decide if we have hardcoded spring constants, or not
    rFlag.Load(flag);
    Dn Zero;
...
#else
#define INSTANCE_KERNEL_Expanded_Collision_Force_Differential(WIDTH) WIDETYPE(float,WIDTH) (&f)[3][8], \
                                                                     const WIDETYPE(float,WIDTH) (&u)[3][8],\
                                                                     const WIDETYPE(float,WIDTH) (&W)[3],\
                                                                     const WIDETYPE(int,WIDTH) (&flag),\
                                                                     const WIDETYPE(float,WIDTH) (&spring_constant), \
                                                                     const WIDETYPE(int,WIDTH) (&spring_id),\
                                                                     const float* extern_collision_stiffness
INSTANCE_KERNEL(Expanded_Collision_Force_Differential);
#undef INSTANCE_KERNEL_Expanded_Collision_Force_Differential
#endif
