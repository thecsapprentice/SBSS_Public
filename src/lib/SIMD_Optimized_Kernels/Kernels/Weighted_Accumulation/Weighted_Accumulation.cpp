...
void Weighted_Accumulation(T_DATA (&u)[3][8], const T_DATA (&F)[9],
                           const T_DATA (&W)[3], const T_DATA (&one_over_h),
                           const T_DATA (&scale) )
{
...
#define M_I(x,y) (y * 3 + x )
    ...
    SCALE_OVER_H.Load(scale);
    SCALE_OVER_H = SCALE_OVER_H * ONE_OVER_H;
    ...
    for( int v = 0; v < 3; v++)
        {
            ...
            RuDII = RuDII * SCALE_OVER_H;
            RuIDI = RuIDI * SCALE_OVER_H;
            RuIID = RuIID * SCALE_OVER_H;
...
#else
#define INSTANCE_KERNEL_Weighted_Accumulation(WIDTH) WIDETYPE(float,WIDTH) (&u)[3][8], const WIDETYPE(float,WIDTH) (&F)[9], const WIDETYPE(float,WIDTH) (&W)[3], const WIDETYPE(float,WIDTH) (&one_over_h), const WIDETYPE(float,WIDTH) (&scale)
