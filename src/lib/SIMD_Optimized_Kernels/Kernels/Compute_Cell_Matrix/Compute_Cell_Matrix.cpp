...
namespace CDC{
    ...
    template<class Tw,class T_DATA=void, class I_DATA=void>
    void Build_M(T_DATA (&M)[9], const T_DATA (&dPdF)[12], const T_DATA (&H)[3][8], int i, int k )
    {
        ...
        /* What goes here??? */
        rM[0] = rhi[0]*rhk[0]*ra1111 + rhi[1]*rhk[1]*ra1212 + rhi[2]*rhk[2]*ra1313;
        rM[1] = rhi[0]*rhk[0]*ra1212 + rhi[1]*rhk[1]*ra2222 + rhi[2]*rhk[2]*ra2323; 
        rM[2] = rhi[0]*rhk[0]*ra1313 + rhi[1]*rhk[1]*ra2323 + rhi[2]*rhk[2]*ra3333; 
        rM[3] = rhi[0]*rhk[1]*ra1122 + rhi[1]*rhk[0]*ra1221;
        rM[4] = rhi[0]*rhk[1]*ra1221 + rhi[1]*rhk[0]*ra1122;
        rM[5] = rhi[0]*rhk[2]*ra1133 + rhi[2]*rhk[0]*ra1331;
        rM[6] = rhi[0]*rhk[2]*ra1331 + rhi[2]*rhk[0]*ra1133;
        rM[7] = rhi[2]*rhk[1]*ra2233 + rhi[1]*rhk[2]*ra2332;
        rM[8] = rhi[2]*rhk[1]*ra2332 + rhi[1]*rhk[2]*ra2233;
                                       ...
                                       T_DATA (&matrix)[300])
{
    ...
    for( int i = 0; i < 8; i++){
        for( int k = 0; k < 8; k++){
            // Build M here ???
            CDC::Build_M<Tw,T_DATA,I_DATA>(Vt, dPdF, H, i, k);
            for( int j=0; j<3; j++)
                for( int l=0; l<3; l++){
                    Tn df_du_value;
                    for( int p=0; p<3; p++)
...
#else
#define INSTANCE_KERNEL_Compute_Cell_Matrix(WIDTH) const WIDETYPE(float,WIDTH) (&one_over_h), const WIDETYPE(float,WIDTH) (&mu_stab), const WIDETYPE(float,WIDTH) (&cell_volume), const WIDETYPE(float,WIDTH) (&U)[9], const WIDETYPE(float,WIDTH) (&V)[9], const WIDETYPE(float,WIDTH) (&dPdF)[12], WIDETYPE(float,WIDTH) (&matrix)[300]
