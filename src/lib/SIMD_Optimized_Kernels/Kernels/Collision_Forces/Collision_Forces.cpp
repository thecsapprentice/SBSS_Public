...
#ifndef SUBROUTINE_Collision_Forces
#include <assert.h>
...
#ifdef SUBROUTINE_Collision_Forces
inline
#endif
void Collision_Forces(T_DATA (&f)[3][8],
                      const T_DATA (&u)[3][8],
                      const T_DATA (&N)[3],
                      const T_DATA (&W)[3],
                      const T_DATA (&h),
                      ...
                      const float* extern_collision_stiffness )
{
    ...
    rAttach[0].Gather( extern_collision_attach, spring_id_X );
    rAttach[1].Gather( extern_collision_attach, spring_id_Y );
    rAttach[2].Gather( extern_collision_attach, spring_id_Z );
...
#else
#define INSTANCE_KERNEL_Collision_Forces(WIDTH) WIDETYPE(float,WIDTH) (&f)[3][8], const WIDETYPE(float,WIDTH) (&u)[3][8], const WIDETYPE(float,WIDTH) (&N)[3], const WIDETYPE(float,WIDTH) (&W)[3], const WIDETYPE(float,WIDTH) (&h), const WIDETYPE(int,WIDTH) (&spring_id), const WIDETYPE(int,WIDTH) (&spring_id_X), const WIDETYPE(int,WIDTH) (&spring_id_Y), const WIDETYPE(int,WIDTH) (&spring_id_Z), const float* extern_collision_attach, const float* extern_collision_stiffness
INSTANCE_KERNEL(Collision_Forces);
#undef INSTANCE_KERNEL_Collision_Forces
#endif
