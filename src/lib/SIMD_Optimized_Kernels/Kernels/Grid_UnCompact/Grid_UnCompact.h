...
template<class T>
class Grid_UnCompact_Task : public PhysBAM::PTHREAD_QUEUE::TASK
{
 private:
 ...
 public:
 Grid_UnCompact_Task(const T (*u_compact)[3][27], const T (*p_compact)[8],  T * uX_ptr,
                     T* uY_ptr,  T* uZ_ptr,  T* p_ptr,
        ...
        for(int block=0;block<_number_of_blocks;block++)
            {
                ...
                for(int i=0;i<3;i++)
                    for(int j=0;j<3;j++)
                        for(int k=0;k<3;k++){
                            ...
                            *(_uX_ptr+index) 
                                += _u_compact[_first_block+block][0][linear_index];
                            *(_uY_ptr+index) 
                                += _u_compact[_first_block+block][1][linear_index];
                            *(_uZ_ptr+index) 
                                += _u_compact[_first_block+block][2][linear_index];
                            linear_index++;
                        }
                ...
                if(_uncompact_p)
                    for(int i=0;i<2;i++)
                        for(int j=0;j<2;j++)
                            for(int k=0;k<2;k++)
                                (_p_ptr+_block_offsets[block])[i*_x_stride_cell+j*_y_stride_cell+k] 
                                    += _p_compact[_first_block+block][4*i+2*j+k];
...
template<class T>
void Grid_UnCompact( const T (*u_compact)[3][27], const T (*p_compact)[8],  T * uX_ptr,
                      T* uY_ptr,  T* uZ_ptr,  T* p_ptr,
                     ...
                     const bool uncompact_p = true)
{
	for(int partition=0;partition<number_of_partitions;partition+=2){
        ...
        Grid_UnCompact_Task<T>* task = new Grid_UnCompact_Task<T>(u_compact,
                                                                  p_compact,
                                                                  uX_ptr,
                                                                  uY_ptr,
                                                                  uZ_ptr,
