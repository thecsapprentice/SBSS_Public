    ...
    KRYLOV_VECTOR_BASE<T>& r,KRYLOV_VECTOR_BASE<T>& z,const T tolerance,const int min_iterations,const int max_iterations)
{
    ...
    LOG::cout<<"SYMMQMR: max_iteration="<<max_iterations<<std::endl;
    LOG::cout<<"SYMMQMR: tolerance="<<tolerance<<std::endl;
    ...
    int iterations;for(iterations=0;;iterations++){
        ...
        if(!theta){
            PHYSBAM_FATAL_ERROR("theta is zero");
        ...
        if((!rho_old || convergence_norm<=tolerance || (iterations && nullspace_measure<=nullspace_tolerance)) &&
            ...
            LOG::cout<<"rho_old="<<rho_old<<std::endl;
            LOG::cout<<"convergence_norm="<<convergence_norm<<std::endl;
            LOG::cout<<"tolerance="<<tolerance<<std::endl;
            LOG::cout<<"iterations="<<iterations<<std::endl;
            LOG::cout<<"nullspace_tolerance="<<nullspace_tolerance<<std::endl;
            LOG::cout<<"nullspace_measure="<<nullspace_measure<<std::endl;
            LOG::cout<<"min_iterations="<<min_iterations<<std::endl;
            LOG::cout<<"small_number="<<small_number<<std::endl;
            LOG::cout<<"residual_magnitude_squared="<<residual_magnitude_squared<<std::endl;
            if(print_diagnostics) LOG::Stat("symmqmr iterations",iterations);if(iterations_used) *iterations_used=iterations;return true;}
