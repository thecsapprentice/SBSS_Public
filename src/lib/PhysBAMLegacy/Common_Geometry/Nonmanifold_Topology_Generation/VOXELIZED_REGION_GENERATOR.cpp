    ...
    for( RANGE_ITERATOR<d-1> iterator(refinement_domain); iterator.Valid(); iterator.Next() )
        {
            ...
            base(d) = 1; //Correct the last position.
...
template <class T, int d> VECTOR<bool,T_CELLTYPE<T, d>::vertices_per_cell> 
VOXELIZED_REGION_GENERATOR<T,d>::CornersInsideOutside( const T_INDEX& cell_index, const int subcell ) const{
    int root_cell_index = BASE::root_index_to_root_id.Get( cell_index );
    int sub_cell_index = root_cell_to_subcell( root_cell_index )(subcell);
    
    VECTOR<bool,T_CELLTYPE<T, d>::vertices_per_cell> corner_flags;
    ARRAY<T_INDEX,T_INDEX> corners;
    RANGE<T_INDEX> voxmap_domain = subcell_voxmaps(sub_cell_index).domain;
    voxmap_domain.Corners(corners);
    
    int flat_index =1;
    for(RANGE_ITERATOR<d> corner_iterator( RANGE<T_INDEX>(T_INDEX(),T_INDEX()+1)); corner_iterator.Valid(); corner_iterator.Next(),flat_index++ ){
        corner_flags(flat_index) = subcell_voxmaps(sub_cell_index)(corners(corner_iterator.Index()));
    }
    return corner_flags;
}
