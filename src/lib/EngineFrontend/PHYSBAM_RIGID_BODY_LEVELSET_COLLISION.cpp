...
#include "PHYSBAM_RIGID_BODY_LEVELSET_COLLISION.h"

using namespace PhysBAM;

template<class T, int d>
PHYSBAM_RIGID_BODY_LEVELSET_COLLISION<T,d>::PHYSBAM_RIGID_BODY_LEVELSET_COLLISION(const std::vector<std::vector<float> > vertices, const std::vector<std::vector<int> > triangles, const float refinement) : grid(NULL), phi(NULL), levelset(NULL)
{
    ...
    levelset_transformed = new IMPLICIT_OBJECT_TRANSFORMED<TV, FRAME<TV> >( levelset, false, &transform );
};

template<class T, int d>  
PHYSBAM_RIGID_BODY_LEVELSET_COLLISION<T,d>::~PHYSBAM_RIGID_BODY_LEVELSET_COLLISION()
{
    if(levelset)
        delete levelset;
    if(levelset_transformed)
        delete levelset_transformed;
    if(grid)
        delete grid;
    if(phi)
        delete phi;
...
PHYSBAM_RIGID_BODY_LEVELSET_COLLISION<T,d>::Phi_Implementation( const T world_location[d]) const
{
    PHYSBAM_ASSERT(levelset_transformed!=NULL);
    VECTOR<T,d> wl;
    for(int i=1;i<=d;i++)
        wl(i) = world_location[i-1];
    return levelset_transformed->Extended_Phi( wl );
};

template<class T, int d> void
PHYSBAM_RIGID_BODY_LEVELSET_COLLISION<T,d>::Normal_Implementation( const T world_location[d], T normal[d]) const
{
    PHYSBAM_ASSERT(levelset_transformed!=NULL);
    VECTOR<T,d> n;
    VECTOR<T,d> wl;
    for(int i=1;i<=d;i++)
        wl(i) = world_location[i-1];
    n = levelset_transformed->Extended_Normal(wl);
    for(int i=1;i<=d;i++)
        normal[i-1] = n(i);
};

template class PHYSBAM_RIGID_BODY_LEVELSET_COLLISION<float, 3>;
template class PHYSBAM_RIGID_BODY_LEVELSET_COLLISION<double, 3>;
//template class PHYSBAM_RIGID_BODY_LEVELSET_COLLISION<float, 2>;
//template class PHYSBAM_RIGID_BODY_LEVELSET_COLLISION<double, 2>;
