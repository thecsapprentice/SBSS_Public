...
//#####################################################################
#ifndef __Vector_SAXPY_A_Helper__
#define __Vector_SAXPY_A_Helper__
...
class Vector_SAXPY_A_Op
    ...
    public:
        explicit Vector_SAXPY_A_Op(T* const x_input, const T* const y_input, const T c_input)
            : x(x_input),y(y_input),c(c_input)  {}
        void Execute(int index)
        {
#pragma vector always
#pragma ivdep
            for(int i=0;i<stride;i++)
                x[index+i] = c * y[index+i];
        }
    };
template <class T> 
    class Vector_SAXPY_A_Op<T,1>
    {
    private:
        T* const x;
        const T* const y;
 ...
 template <class T> inline
     void Vector_SAXPY_A_Helper( T* const x_input,  const T* const y_input, const T c_input,
     ...
     {
         Vector_SAXPY_A_Op<T> op(x_input, y_input, c_input);
         Kernel_Base_Helper<Vector_SAXPY_A_Op<T> > helper(op, block_offsets_input,  number_of_blocks_input,
 ...
 template <class T, class LAYOUT> inline
     ...
     {
         Vector_SAXPY_A_Op<T> op(x_input, y_input, c_input);
         Kernel_Mesh_Base_Helper<Vector_SAXPY_A_Op<T>, LAYOUT > helper(op, block_layouts_input,  number_of_blocks_input,
                                                        partition_offsets_input,  number_of_partitions_input,
                                                        x_stride_input, y_stride_input, block_size_input);
         helper.Run_Parallel();
     };
 ...
 template <class T> inline
     ...
     {
         Vector_SAXPY_A_Op<T> op(x_input, y_input, c_input);
         Kernel_Serial_Base_Helper<Vector_SAXPY_A_Op<T> > helper(op, number_of_entries_input,
...
#if 0
     Vector_SAXPY_A_Op<T,stride> op(x_input, y_input, c_input);
     Kernel_OpenMP_Base_Helper<Vector_SAXPY_A_Op<T,stride> >helper(op, offsets_input, 
...
#pragma omp parallel for num_threads(number_of_partitions_input)
     for(int i=0;i<number_of_offsets_input;i++){
         ...
         for(int j=0;j<stride;j++)
             x_input[offset+j] = c_input * y_input[offset+j];
    }
